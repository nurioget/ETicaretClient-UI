...
export class ListComponent extends BaseComponent implements OnInit {
  ...
  async getOrders() {
    ...
    const allOrders: { totalOrderCount: number; orders: List_Order[] } = await
      this.orderService.getAllOrders(this.paginator ? this.paginator.pageIndex : 0, this.paginator ?
        this.paginator.pageSize : 5, () => this.hideSpinner(SpinnerType.BallAtom), errorMessage => this.alertifyService.message(errorMessage, {
          dismissOthers: true,
  ...
  showDetail(id: string) {
    this.dialogService.openDialog({
      componnentType: OrderDetailDialogComponent,
      data: id,
      options: {
        width: "750px"
      }
